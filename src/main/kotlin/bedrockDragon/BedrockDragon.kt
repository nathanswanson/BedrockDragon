...
fun main() {
    ...
    logger.info { "Validating server root" }
        ...
        try { FileReader("mods/") } catch (e: FileNotFoundException) { File("mods").mkdir()}
        try { FileReader("worlds/") } catch (e: FileNotFoundException) { File("worlds").mkdir()}
        try { FileReader("players/") } catch (e: FileNotFoundException) { File("players").mkdir()}
        try { FileReader("logs/") } catch (e: FileNotFoundException) { File("logs").mkdir()}
        try { FileReader("configs/") } catch (e: FileNotFoundException) { File("configs").mkdir()}


        val serverPropeties: Properties = Properties()
            try {
                ...
                serverPropeties.store(FileOutputStream("server.properties"), "DragonServer")
